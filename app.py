...
# Initialize Data:
dd, jdd = Tools.Date.getRange(config["dd"][0], config["dd"][1], config["dinc"]) # Departure dates
ad, jad = Tools.Date.getRange(config["ad"][0], config["ad"][1], config["dinc"]) # Arrival dates
...
(rP0, vP0, rP1, vP1,
 c3s0, c3s1, c3l0, c3l1, 
 dVs0, dVs1, dVl0, dVl1, tofs) = Tools.Transfer.solve(config["k"], Body0, Body1, jdd, jad)
...
in_s, in_l = Tools.Plot.pick_plot_type(config["plt_type"], c3s0, c3s1, c3l0, c3l1, dVs0, dVs1, dVl0, dVl1)
st.plotly_chart(Tools.Plot.porkchop(config, dd, ad, in_s, in_l, tofs), use_container_width=True)